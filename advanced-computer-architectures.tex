...
\begin{tabular}{m{30em} m{14em}}
  ...
  %\begin{figure}[h!]
  %  \centering
    \includegraphics[scale=0.25]{Branch_Target_Buffer.png}
...
\subsubsection{Branch Hazards: Example}
...
started.
\begin{itemize}
  \item If the branch is not taken, the pipeline execution is OK
  \item If the branch is taken, it is necessary to flush the next 3 instructions
  in the pipeline and fetched the lw instruction at the branch target
  address (L1) 
\end{itemize}

\subsubsection{Branch Stalls without Forwarding}
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.4]{Branch_Stalls_without_Forwarding.png}
